#!/usr/bin/env python3
"""
Asethetics from https://github.com/0xalizk/mopro-benchmarks/blob/main/src/main.py


Generate RPC call distribution bar plot showing PERCENTAGE distribution from aggregate statistics.
"""
...
def beautify_ax(ax, colors, labels, groups, title, x_label, y_label, xticks):
    ...
    ax.set_xticklabels(groups, fontsize=26, fontproperties=ubuntu_font)
...
def main():
    ...
    # Absolute data from aggregate files:
    # Aave: State=9, Receipts=33, Transactions=1, Blocks=2 -> Total=45
    # Helios: State=53, Receipts=59, Transactions=77, Blocks=28 -> Total=217
    # Rotki: State=8, Receipts=4, Transactions=6, Blocks=3 -> Total=21
    # Kohaku: State=12, Receipts=6, Transactions=6, Blocks=5 -> Total=29
    # Zerion: State=9, Receipts=7, Transactions=9, Blocks=5 -> Total=30
    ...
    # Calculate totals for each codebase
    totals = []
    for i in range(len(codebases)):
        total = sum(data_absolute[cat][i] for cat in categories)
        totals.append(total)

    # Convert to percentages
    data_percent = {}
    for cat in categories:
        data_percent[cat] = [(data_absolute[cat][i] / totals[i]) * 100 for i in range(len(codebases))]
    ...
    for category, color in zip(categories, colors):
        ...
        bars = ax.bar(x + offset, data_percent[category], width, label=category, color=color, edgecolor='white', linewidth=0.5)
    ...
    beautify_ax(
        ...
        labels=categories,
        groups=codebases,
        title='RPC Call Distribution (Relative)',
        x_label='Context',
        y_label='% of RPC call triggers in each codebase',
        xticks=xticks
    )
    ...
    plt.tight_layout()
    plt.savefig('rpc-dist-relative.png', dpi=150, bbox_inches='tight', facecolor='white')
    print("Saved plot to rpc-dist-relative.png")
